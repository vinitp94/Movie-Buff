...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(:actors).where("name = 'Harrison Ford'").where("ord != 1")
end
...
def directed_by_one_of(them)
  ...
  Movie.select(:id, :title).joins("JOIN actors ON actors.id = movies.director_id").where("actors.name IN (?)", them)
end
